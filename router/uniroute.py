...
class UniRouteRouter:
    ...
    def __init__(
        ...
        Raises:
            RouterColdStartError: If registry is empty or cluster_assigner has
                no centroids — caller should fall back to a default model.
            ValueError: If allowed_models contains IDs not in the registry.
        """
        if len(registry) == 0:
            raise RouterColdStartError(
                "LLMRegistry is empty — cannot route. "
                "Register at least one LLMProfile or fall back to a default model."
            )
        if cluster_assigner.num_clusters == 0:
            raise RouterColdStartError(
                "ClusterAssigner has 0 centroids — no fit yet. "
                "Wait for the harness to fit clusters or fall back to a default model."
            )
    ...
    def route(
        ...
        Raises:
            ValueError: If no models are available after filtering.
        """
