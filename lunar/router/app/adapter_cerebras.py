...
from .helpers.secrets_config import is_byok_required, get_cerebras_key_for_tenant, DEFAULT_CEREBRAS_API_KEY


class CerebrasAdapter(ProviderAdapter):
    """
    Adapter for Cerebras models via LiteLLM with:
    ...
    def _count_tokens(self, messages, completion_text: str) -> Tuple[int, int]:
        # tokens in - use a generic model for token counting since Cerebras models may not be recognized
        try:
    ...
    async def send(self, req: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, float]]:
        ...
        try:
            api_key = get_cerebras_key_for_tenant(tenant_id, byok_required)
        ...
        if not api_key:
            return (
                {"error": "No Cerebras API key available for this environment"},
                {
                    "ttft_ms": 0.0,
                    "latency_ms": 0.0,
                    "tokens_in": 0,
        ...
        try:
            ...
            # Extract the model name without the cerebras/ prefix
            actual_model = self.model_name
            if actual_model.startswith("cerebras/"):
                actual_model = actual_model[9:]  # Remove "cerebras/" prefix
    ...
    def healthy(self) -> Dict[str, Any]:
        ...
        """
        has_global = bool(DEFAULT_CEREBRAS_API_KEY)
        return {
            "ok": has_global,
