...
from .helpers.secrets_config import is_byok_required, get_cohere_key_for_tenant, DEFAULT_COHERE_API_KEY


class CohereAdapter(ProviderAdapter):
    """
    Adapter for Cohere models via LiteLLM with:
    ...
    async def send(self, req: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, float]]:
        ...
        try:
            api_key = get_cohere_key_for_tenant(tenant_id, byok_required)
        ...
        if not api_key:
            return (
                {"error": "No Cohere API key available for this environment"},
                {
                    "ttft_ms": 0.0,
                    "latency_ms": 0.0,
                    "tokens_in": 0,
    ...
    def healthy(self) -> Dict[str, Any]:
        ...
        """
        has_global = bool(DEFAULT_COHERE_API_KEY)
        return {
            "ok": has_global,
