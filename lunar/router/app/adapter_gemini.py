...
from .helpers.secrets_config import is_byok_required, get_gemini_key_for_tenant, DEFAULT_GEMINI_API_KEY


class GeminiAdapter(ProviderAdapter):
    """
    Adapter for Google Gemini models via LiteLLM with:
    ...
    async def send(self, req: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, float]]:
        ...
        try:
            api_key = get_gemini_key_for_tenant(tenant_id, byok_required)
        ...
        if not api_key:
            return (
                {"error": "No Gemini API key available for this environment"},
                {
                    "ttft_ms": 0.0,
                    "latency_ms": 0.0,
                    "tokens_in": 0,
    ...
    def healthy(self) -> Dict[str, Any]:
        ...
        """
        has_global = bool(DEFAULT_GEMINI_API_KEY)
        return {
            "ok": has_global,
