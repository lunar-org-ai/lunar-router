...
from .helpers.secrets_config import is_byok_required, get_anthropic_key_for_tenant, DEFAULT_ANTHROPIC_API_KEY
...
class AnthropicAdapter(ProviderAdapter):
    """
    Adapter for Anthropic Claude models via LiteLLM with:
    ...
    async def send(self, req: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, float]]:
        ...
        try:
            api_key = get_anthropic_key_for_tenant(tenant_id, byok_required)
        ...
        if not api_key:
            return (
                {
                    "error": "No Anthropic API key available for this environment",
                    "error_category": ErrorCategory.AUTH_ERROR.value,
                    "error_details": {
    ...
    def healthy(self) -> Dict[str, Any]:
        ...
        """
        has_global = bool(DEFAULT_ANTHROPIC_API_KEY)
        return {
            "ok": has_global,
