"""
Go Engine Client: Thin Python SDK for the OpenTracy Go backend.
...
class GoEngine:
    ...
    Starts opentracy-engine as a subprocess, communicates via localhost HTTP.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            binary_path: Path to opentracy-engine binary. Auto-detected if None.
...
def _find_binary() -> str:
    """
    Find the opentracy-engine binary, checking:
    1. go/bin/opentracy-engine in the project (dev mode)
    2. Bundled in the package (_bin/)
    3. PATH
    """
    # Dev mode: built binary in go/bin/
    project_root = Path(__file__).parent.parent
    dev_binary = project_root / "go" / "bin" / "opentracy-engine"
    if dev_binary.exists():
        return str(dev_binary)

    # Bundled in package
    pkg_binary = Path(__file__).parent / "_bin" / f"opentracy-engine-{_platform_tag()}"
    if pkg_binary.exists():
        return str(pkg_binary)

    # On PATH
    path_binary = shutil.which("opentracy-engine")
    if path_binary:
        return path_binary

    raise FileNotFoundError(
        f"opentracy-engine binary not bundled for this platform ({_platform_tag()}).\n"
        f"Supported platforms get the binary automatically via `pip install opentracy`.\n"
        f"If you hit this, options:\n"
        f"  1. File an issue — https://github.com/OpenTracy/opentracy/issues\n"
...
def _find_bundled_onnxruntime() -> Optional[Path]:
    ...
    Ship the library inside ``opentracy/_bin/`` so users don't need to
...
def _ensure_onnx_assets(weights_path: str) -> None:
    ...
    ``opentracy/_onnx/``, copy them on first start so the user never has to
...
def _find_weights() -> str:
    ...
    # macOS
    app_support = Path.home() / "Library" / "Application Support" / "opentracy"
    ...
    # Linux
    local_share = Path.home() / ".local" / "share" / "opentracy"
