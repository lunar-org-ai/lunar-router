"""OpenTracy SDK — OpenAI-compatible client across 13 providers.

This module is the plumbing behind ``opentracy.completion`` and
``opentracy.Router``. It parses ``provider/model`` strings, resolves a
target (direct-to-provider HTTP, OpenTracy engine, or explicit ``api_base``),
builds an OpenAI-schema request body, and wraps the response with OpenTracy
metadata (``_cost``, ``_latency_ms``, ``_provider``, ``_routing``).

The SDK is the entry point for the broader OpenTracy loop: every request can be
...
aliases — without your code changing. See ``opentracy`` package docs.
...
Engine mode (drop-in OpenAI SDK, zero code changes):
    ...
    # All providers routed through the OpenTracy engine; each request is a trace.
"""
...
from opentracy._env import env, env_in_environ
...
class ModelResponse(_AttrDict):
    """OpenAI-compatible chat completion response with OpenTracy metadata.

    Standard fields: id, object, model, choices, usage
    OpenTracy extras:  _provider, _cost, _latency_ms, _routing
    """
    pass
...
def _build_model_response(
    ...
    """Build a ModelResponse from a raw dict, adding OpenTracy metadata."""
    # Calculate cost
    cost = 0.0
    ...
    resp["_routing"] = {
        k.replace("X-OpenTracy-", "").lower().replace("-", "_"): v
        for k, v in data.items()
        if isinstance(k, str) and k.startswith("X-OpenTracy-")
    }
    return resp
