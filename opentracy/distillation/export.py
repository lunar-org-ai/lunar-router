...
Usage (subprocess):
    python -m opentracy.distillation.export /path/to/config.json
...
async def start_export(
    ...
    proc = await asyncio.create_subprocess_exec(
        sys.executable, "-m", "opentracy.distillation.export",
        str(config_path),
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m opentracy.distillation.export <config.json>")
        sys.exit(1)
