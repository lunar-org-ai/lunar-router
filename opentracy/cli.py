...
Usage:
    ...
    $ opentracy remove weights-mmlu-v1
    $ opentracy path weights-mmlu-v1

Or via Python:
    $ python -m opentracy download weights-mmlu-v1
"""
...
def _migrate_legacy_config_dir() -> None:
    """One-time migration: copy ~/.lunar/ to ~/.opentracy/ for users upgrading
    from lunar-router. Prints a one-line info message; does not delete the old
    directory so users can verify and remove it themselves.
    """
    legacy = Path.home() / ".lunar"
    new = Path.home() / ".opentracy"
    if legacy.exists() and not new.exists():
        import shutil
        try:
            shutil.copytree(legacy, new)
            print(
                f"[opentracy] Migrated {legacy} -> {new}. "
                f"Remove the old directory once you've confirmed the move.",
                file=sys.stderr,
            )
        except OSError as e:
            print(
                f"[opentracy] Could not migrate {legacy}: {e}. "
                f"Copy it manually: cp -r {legacy} {new}",
                file=sys.stderr,
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="opentracy",
        description="OpenTracy - Intelligent LLM Routing",
    )
    parser.add_argument(
        "--version",
        action="version",
